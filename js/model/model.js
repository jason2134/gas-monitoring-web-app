...
//SENSOR NODES onclick event to show datas
      function initNode1() 
      {
        // create a cube by three.js, not using .stl file.
        var geometry = new THREE.BoxGeometry(10, 10, 10);//cube shape
        var material = new THREE.MeshLambertMaterial({   //cube colour
          color: 0xef6500,
          needsUpdate: true,
          opacity: 1,
          transparent: true,
        });
        // mesh to initialize the cube setting
        var Node1 = new THREE.Mesh(geometry, material);
        Node1.position.set(10, 1.5, 0);
        Node1.castShadow = true;
        // scene.add to make sure node 1 can be seen on canvas
        scene.add(Node1); 
        //add cursor to activate mouser change display
        Node1.cursor = "pointer";
        Node1.name = "node1";
        //this is used to activate onclick event, which will be linked to loopNode1() in ops.js
...
function onClickOverall() {
  ...
  initOverallNode4();
  overallIcon();
  document.getElementById("dashboardTitle").innerHTML = "AIR QUALITY MONITORING";
}
...
function onClickF1() {
  ...
  initNode1();
  f1Icon();
  document.getElementById("dashboardTitle").innerHTML = "AIR QUALITY MONITORING - F1";
}
...
function onClickF2() {
  ...
  initF2Node3();
  f2Icon()
  document.getElementById("dashboardTitle").innerHTML = "AIR QUALITY MONITORING - F2";
}
...
function onClickF3() {
  ...
  initF3();
  initF3Node4();
  document.getElementById("dashboardTitle").innerHTML = "AIR QUALITY MONITORING - F3";
}
  ...
  function initGui() 
  {
    ...
    //将设置属性添加到gui当中，gui.add(对象，属性，最小值，最大值）,for position parameter debugging.
    var obj = {};
  }
